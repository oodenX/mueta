...
def test_cache_search_results(tmp_path):
    ...
    # Use actual API: set_search_result(query, artist, strict, recording_id)
    cache.set_search_result("test query", "test artist", True, "result-mbid-456")

    # get_search_result(query, artist, strict)
    result = cache.get_search_result("test query", "test artist", True)
